...
router.get('/', checkAuth, (req, res, next) => {
    User.find()
    .select("user_name email link_profile full_name")
    .exec()
    .then(result => {
        const response = {
            status: 200,
...
router.get('/:id_user', checkAuth, (req, res, next) => {
    ...
    .select("_id user_name email rules full_name created_on")
    .then(result => {
        if(result) {
            const response = {
...
router.patch('/:id_user', checkAuth, (req, res, next) => {
    ...
    User.findByIdAndUpdate(id_user, req.body, {runValidators: true} ,(error, result) => {
